...
void pickUpItem(int next_step) {
    ...
    if ( err < 0.1) {
        //Only as we get close to the target set the orientation
        //maintaining orientation throughout the approach to
        //the item, uses up fuel
        ...
        if ( dockVelocityOk() && dockOrientOk(orient) && err < 0.02 ) {
            ...
            ;
            #ifdef VERBOSE
            DEBUG(("Item-Pickup:Moving too fast or too far"));
            #endif
    }
}
void loop(){
