...
5. Wait at zone until 2nd large item is available. Stealing
   from opponent's zone if need be.
   Chose this strategy since sometimes while we are picking up
   the smaller items, the opponent comes and steals our large item.
   Staying at the zone, should also help block the opponent to
   some extent.
Change from version 2.2
6. Sometimes the opponent snuck in and picked up the item we were
...
7. Increased the angle error to 0.24. Decreased the zoneErrorTolerance
   to 0.02, this was because sometimes even after dropping the item in
   the zone, it wasn't accurate enough to accrue points.
8. For the position PD values, as fuel dropped increased D from 3 to 5.
9. In getClosestAvailableItem() function also update array itemDist[],
   which stores the distances from the sphere to all items searched for.
10. Code size is 89% of max allowable.
